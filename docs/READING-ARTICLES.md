...
- 各文そのものをタップ対象にし、タップした文の直下に日本語訳を差し込む。個別の「訳」ボタンは置かない。
- 構造データがある英文は、意味・文法上の塊ごとに文字色で区別する。チャンク自体はタップ対象にしない。
...
- `predicate`: 述部。藍系の文字色。
- `object`: 目的語。水系の文字色。
- `complement`: 補語。水系の文字色。
- `clause`: 条件節・that節・疑問詞節など。土系の文字色。
- `relative`: 関係詞節、関係詞省略の節。土系の文字色。
- `infinitive`: to不定詞。土系の文字色。
- `participle`: 分詞・分詞句。土系の文字色。
- `modifier`: 副詞句、前置詞句、比較句などの修飾。控えめな文字色。
- `connector`: but / yet / so / and など論理のつなぎ。強めのアクセント色。

色分けは、細かな文法用語を暗記するためではなく、**英文を前から意味・文法上の塊として処理するための視覚補助**です。チャンクは解説UIやボタンにせず、本文の一部として自然に読めることを優先します。

特にスマホでは、1つのチャンクが複数行に折り返すことがあります。背景塗りや長い下線で塊を囲わず、ライト/ダーク双方でコントラストを確保した文字色で区別してください。
...
1. 一文そのものをタップすると、その文の直下に日本語訳が開閉する。個別の「訳」ボタンは置かない。
2. 構造チャンクは色分けだけにし、タップ解説や個別ボタンにしない。
3. 複数行に折り返すチャンクも、同じ塊として自然に読める見た目にする。
4. 構造色は背景に埋もれない文字色を基本とし、ライト/ダーク双方で可読性を優先する。
5. 精読情報を読むためにモード切替を要求しない。
6. フローティングパネルを教材の主導線にしない。
7. Kokeniwa の既存CSS変数・ライト/ダークテーマ・苔庭の世界観に合わせる。
8. モバイルでも本文の可読性を最優先する。
